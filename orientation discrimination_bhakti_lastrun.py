...
expName = 'orientation discrimination_bhakti'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/bhaktigajjar/Desktop/AhdUni/lap psychology/Orientation Discrimination task/orientation discrimination_bhakti_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    for thisTrial in trials:
        ...
        trial = data.Routine(
            name='trial',
            components=[fixation, key_resp, grating_2],
        )
        ...
        while continueRoutine:
            ...
            if key_resp.status == STARTED and not waitOnFlip:
                ...
                if len(_key_resp_allKeys):
                    ...
                    # was this correct?
                    if (key_resp.keys == str(corrAnswer)) or (key_resp.keys == corrAnswer):
                        key_resp.corr = 1
                    else:
                        key_resp.corr = 0
            ...
            if grating_2.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
                # keep track of start time/frame for later
            ...
            if grating_2.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > grating_2.tStartRefresh + 0.3-frameTolerance:
                    # keep track of stop time/frame for later
                    grating_2.tStop = t  # not accounting for scr refresh
                    grating_2.tStopRefresh = tThisFlipGlobal  # on global time
